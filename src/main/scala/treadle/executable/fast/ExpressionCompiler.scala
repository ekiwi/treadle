...
class ExpressionCompiler(
  ...
  def makeAnd(e1: ExpressionResult, e2: ExpressionResult, resultWidth: Int): ExpressionResult = (e1, e2) match {
    case (a: IntExpressionResult, b: IntExpressionResult) => {
      AndInts(a.apply, b.apply, resultWidth)
    }
            ...
            getDrivingClock(clockExpression) match {
              case Some(clockSymbol) =>
                ...
                def argExpr(e: ExpressionResult) : PrintfArgExpressionResult = e match {
                  case i : IntExpressionResult  => PrintfArgExpressionResult(i.apply)
                  case l : LongExpressionResult => PrintfArgExpressionResult(l.apply)
                  case b : BigExpressionResult  => PrintfArgExpressionResult(b.apply)
                  case _ =>
                    throw TreadleException(s"Error: printf $printf has unknown condition type")
                }
                val printOp = PrintfOp(
                  ...
                  info, stringLiteral,
                  argExpressions.map { expression => argExpr(processExpression(expression)) },
                  clockTransitionGetter,
                  boolExpression
                )
                addAssigner(printOp)
