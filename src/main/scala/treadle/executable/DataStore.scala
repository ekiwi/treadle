...
def makeConstantAssigner(symbol: Symbol, value: Big, info: Info) : Assigner = {
    symbol.dataSize match {
      case IntSize  => AssignInt(symbol,  GetIntConstant(value.toInt).apply, info)
      case LongSize => AssignLong(symbol, GetLongConstant(value.toLong).apply, info)
      case BigSize  => AssignBig(symbol,  GetBigConstant(value).apply, info)
    }
  }
