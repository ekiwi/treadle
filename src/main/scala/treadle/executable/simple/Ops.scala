...
case class NeqBigs(f1: FuncBig, f2: FuncBig) extends BigExpressionResult {
  def apply(): Big = if(f1() != f2()) 1 else 0
}

case class LtBigs(f1: FuncBig, f2: FuncBig) extends BigExpressionResult {
  def apply(): Big = if(f1() < f2()) 1 else 0
}

case class LeqBigs(f1: FuncBig, f2: FuncBig) extends BigExpressionResult {
  def apply(): Big = if(f1() <= f2()) 1 else 0
}
...
case class GeqBigs(f1: FuncBig, f2: FuncBig) extends BigExpressionResult {
  def apply(): Big = if(f1() >= f2()) 1 else 0
}
...
case class AsClockBigs(f1: FuncBig) extends BigExpressionResult {
  def apply(): Big = if(f1() == 0) 0 else 1
}
...
/**
  ...
  */
case class AndrBigs(f1: FuncBig, width: Int) extends BigExpressionResult {
...
/**
  ...
  * @param width result bit size
  */
case class OrrBigs(f1: FuncBig, width: Int) extends BigExpressionResult {
  private val bitMask = BitMasks.getBitMasksBigs(width).allBitsMask
...
/**
  ...
  * @param width result bit size
  */
case class XorrBigs(f1: FuncBig, width: Int) extends BigExpressionResult {
  private val bitMask = BitMasks.getBitMasksBigs(width).allBitsMask
