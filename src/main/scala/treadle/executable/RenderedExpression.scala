...
class SymbolAtDepth(val symbol: Symbol, val displayDepth: Int, val dataTime: Long, val dataArrays: DataStoreReader)

object SymbolAtDepth {
  def apply(symbol: Symbol, displayDepth: Int, dataTime: Long, dataArrays: DataStoreReader): SymbolAtDepth = {
...
class ExpressionViewRenderer(
  ...
  private def renderInternal(
    ...
    def renderView(view: ExpressionView, displayDepth: Int, dataTime: Long, dataArrays: DataStoreReader): String = {
      val builder = new StringBuilder()
